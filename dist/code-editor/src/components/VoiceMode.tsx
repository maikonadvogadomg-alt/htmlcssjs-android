...
import { speak, stopSpeaking, TTSConfig } from "@/lib/tts-service";

interface VoiceModeProps {
  onClose: () => void;
  onSend: (text: string) => Promise<string>; // retorna resposta já como texto
  ttsConfig: TTSConfig;
}
...
export default function VoiceMode({ onClose, onSend, ttsConfig }: VoiceModeProps) {
  ...
  const [heard, setHeard]   = useState("");   // o que o usuário falou
  const [reply, setReply]   = useState("");   // o que a IA respondeu
  ...
  const listen = useCallback(() => {
    ...
    rec.onend = async () => {
      ...
      try {
        ...
        setReply(clean.slice(0, 400));
        setPhaseSync("speaking");

        const utt = speak(clean, ttsConfig);
        const afterSpeak = () => {
          if (autoLoop) {
  ...
  const handleButton = useCallback(() => {
    if (phase === "idle") {
      ...
      recRef.current?.stop(); // força envio do que capturou
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#080e08]/96 backdrop-blur-md flex flex-col items-center justify-between p-6">
      ...
      <div className="w-full flex items-center justify-between">
        ...
        <button onClick={() => { recRef.current?.stop(); stopSpeaking(); if (loopTimer.current) clearTimeout(loopTimer.current); onClose(); }}
